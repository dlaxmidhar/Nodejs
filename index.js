...
async function SendEmailQuery(_,{email}){
  try {
    ...
    if (matchingDoc) {
      ...
      matchingDoc['@metadata']['@expires'] = new Date(Date.now() + 10 * 60 * 1000).toISOString();
...
async function cred(_,{userotp,email}){
...
async function validatetoken(_,{email,token}){
...
let final="false";
  ...
  console.log()
  await auth.getUser(useruid).then((userrecord)=>{
  if(userrecord.email==email){
    console.log("sucess"+userrecord.email)
    final="true";
  }else{
    console.log("fail")
  final="false";
